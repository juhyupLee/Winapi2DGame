...
CPlayer::CPlayer(IBaseScene* pScene)
    :
    IBaseObject(pScene),
    m_iStatus(0),
    m_iDirection(0),
...
void CPlayer::Render()
{
    ...
    // HP 게이지 
    //----------------------------------------
    SINGLETON(CBackBuffer)->DrawSprite(m_iX - 30, m_iY - 120, SINGLETON(CSpriteManager)->GetSprite(IBaseObject::HP_BAR, 0, 0, 0), 0x00ffffff);
    //----------------------------------------
    // 그림자 스프라이트
    //----------------------------------------
